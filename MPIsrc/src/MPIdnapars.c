...
/* MPIdnapars is a modification of the phylip dnapars program.
   ...
   MPIdnapars was developed in June 2009 by Alexander J. Ropelewski, 
...
{  /* DNA parsimony by uphill search */
  ...
  MPICopyWholeFile("stdin", "stdin file", "MPIdnapars");            /*-MPI-*/
  MPI_Barrier(MPI_COMM_WORLD);                                      /*-MPI-*/
 
  MPIOpenStdinFile(stdin,"stdin","stdin file","r", "MPIdnapars",    /*-MPI-*/
              stdinfilename);                                       /*-MPI-*/
  MPI_Barrier(MPI_COMM_WORLD);                                      /*-MPI-*/
  /***************************************************************** *-MPI-*/
  init(argc, argv);
  ...
  MPICopyWholeFile(infilename, "input file", argv[0]);              /*-MPI-*/
  MPI_Barrier(MPI_COMM_WORLD);                                      /*-MPI-*/
  ...
  /* openfile(&infile,INFILE,"input file", "r",argv[0],infilename); *//*orig*/
  /* openfile(&outfile,OUTFILE,"output file", "w",argv[0],outfilename);*//*orig*/
  ...
  if (weights || justwts) {                                           /*-MPI-*/
    MPICopyWholeFile(WEIGHTFILE, "weights file", "MPIdnapars");      /*-MPI-*/
    MPI_Barrier(MPI_COMM_WORLD);                                     /*-MPI-*/
  ...
  /* THIS WAS MOVED FROM THE INNER LOOP FOR MPI VERSION */
  /*if (!(justwts && !firstset)) */                             /*-MPI-*/
  /*    allocrest();                 */                         /*-MPI-*/
  /***********************************************************************/
  /* MPI Code additions are below                                        */
  /***********************************************************************/
