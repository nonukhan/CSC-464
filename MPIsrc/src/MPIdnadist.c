#include <mpi.h>          /*-MPI-*/
#include <sys/types.h>    /*-MPI DEBUG TIMES */
#include <time.h>         /*-MPI DEBUG TIMES */
...
void getoptions()
{
   ...
   * programs */
  if (similarity && matrix_flags == MAT_MACHINE)
    matrix_flags = MAT_HUMAN;
...
{  /* DNA Distances by Maximum Likelihood */
  ...
  MPICopyWholeFile("stdin", "stdin file", "MPIdnadist");            /*-MPI-*/
  MPI_Barrier(MPI_COMM_WORLD);                                      /*-MPI-*/
 
  MPIOpenStdinFile(stdin,"stdin","stdin file","r", "MPIdnadist",    /*-MPI-*/
              stdinfilename);                                       /*-MPI-*/
  MPI_Barrier(MPI_COMM_WORLD);                                      /*-MPI-*/
  /***************************************************************** *-MPI-*/
  init(argc, argv);
  ...
  ttratio0 = ttratio;
  if (ctgry) {
    MPICopyWholeFile(CATFILE, "categories file", "MPIdnadist");      /*-MPI-*/
    MPI_Barrier(MPI_COMM_WORLD);                                     /*-MPI-*/
  ...
  if (weights || justwts) {
    MPICopyWholeFile(WEIGHTFILE, "weights file", "MPIdnadist");      /*-MPI-*/
    MPI_Barrier(MPI_COMM_WORLD);                                     /*-MPI-*/
