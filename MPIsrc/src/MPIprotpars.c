...
#include "mpi.h"  /*-MPI-*/
#include "MPIroutines.h" /*-MPI-*/

/* MPIprotpars is a modification of the phylip protpars program.
   The modifications enable the program to be run in parallel on 
   a workstation cluster that either does or does not share a common 
   file system but has the MPI message passing system.

   MPIprotpars was developed in January 2006 by Alexander J. Ropelewski, 
  ...
  for (m = 0; m < chars; m++) {
    ...
    if (left == NULL) {
      ...
      if (ls[0] == rs[0]) {
        ...
        switch (i) {
          case 1:
            qs[1] = (ls[0] & rs[1]) | (ls[1] & rs[0]);
            qs[2] = (ls[0] & rs[2]) | (ls[1] & rs[1]) | (ls[2] & rs[0]);
            break;
          case 2:
            qs[1] = (ls[0] & rs[2]) | (ls[1] & rs[1]) | (ls[2] & rs[0]);
            qs[2] = ls[0] | (ls[1] & rs[2]) | (ls[2] & rs[1]) | rs[0];
            break;
          case 3:
            qs[1] = ls[0] | (ls[1] & rs[2]) | (ls[2] & rs[1]) | rs[0];
            qs[2] = fullset;
            break;
          case 4:
            qs[1] = fullset;
            qs[2] = fullset; /* should this be fulldel */
            break;
        }
        for (i = 0; i <= 1; i++) {
...
{  /* Protein parsimony by uphill search */
...
#endif
  /*************************************************************************/
  /* MPI Code additions are below                                          */
  /*************************************************************************/
  char stdinfilename[100];                                          /*-MPI-*/
  FILE *tmpstdin;                                                   /*-MPI-*/
  MPIInitialize(&argc,argv);                                        /*-MPI-*/
  MPICopyWholeFile("stdin", "stdin file", argv[0]);                 /*-MPI-*/
  MPI_Barrier(MPI_COMM_WORLD);                                      /*-MPI-*/
  MPIOpenFile(&tmpstdin,"stdin","stdin file","r", argv[0],          /*-MPI-*/
              stdinfilename);                                       /*-MPI-*/
  stdin=tmpstdin;                                                   /*-MPI-*/
  MPI_Barrier(MPI_COMM_WORLD);                                      /*-MPI-*/
  /***************************************************************/ /*-MPI-*/
  ...
  progname = argv[0];
  /*************************************************************************/
  /* MPI Code additions/changes are below                                  */
  /*************************************************************************/
  MPICopyWholeFile(INFILE, "input file", argv[0]);                  /*-MPI-*/
  MPIOpenFile(&infile,INFILE,"input file","r",argv[0],infilename);  /*-MPI-*/
  MPI_Barrier(MPI_COMM_WORLD);                                      /*-MPI-*/
  MPIOpenFile(&outfile,OUTFILE,"output file", "w", argv[0],         /*-MPI-*/
              outfilename);                                         /*-MPI-*/
  MPI_Barrier(MPI_COMM_WORLD);                                      /*-MPI-*/
  /*  openfile(&infile,OUTFILE,"input file","r",argv[0],        */ /*-Orig-*/
  /*           infilename);                                     */ /*-Orig-*/
  /*  openfile(&outfile,OUTFILE,"output file","w",argv[0],      */ /*-Orig-*/
  /*           outfilename);                                    */ /*-Orig-*/
  /***************************************************************/ /*-MPI-*/
  ibmpc = IBMCRT;
  ansi = ANSICRT;
  ...
  setup();
  doinit();
  /* if (weights || justwts)                                      */ /*-Orig-*/
  /*   openfile(&weightfile,WEIGHTFILE,"weights file","r",argv[0],*/ /*-Orig-*/
  /*            weightfilename);                                  */ /*-Orig-*/
  if (weights || justwts) {                                           /*-MPI-*/
    MPICopyWholeFile(WEIGHTFILE, "weights file", argv[0]);            /*-MPI-*/
    MPIOpenFile(&weightfile,WEIGHTFILE,"weights file","r",argv[0],    /*-MPI-*/
                weightfilename);                                      /*-MPI-*/
    MPI_Barrier(MPI_COMM_WORLD);                                      /*-MPI-*/
    }                                                                 /*-MPI-*/
  /* if (trout)                                                   */ /*-Orig-*/
  /*   openfile(&outtree,OUTTREE,"output tree file","w",argv[0],  */ /*-Orig-*/
  /*            outtreename);                                     */ /*-Orig-*/
  if (trout) {                                                        /*-MPI-*/
    MPIOpenFile(&outtree,OUTTREE,"output tree file","w",argv[0],      /*-MPI-*/
                outtreename);                                         /*-MPI-*/
    MPI_Barrier(MPI_COMM_WORLD);                                      /*-MPI-*/
    }                                                                 /*-MPI-*/
  /* MPICheckDatasets(datasets); not sure what this did */            /*-MPI-*/
  /****************************************************************/  /*-MPI-*/
  /* Because this is parallel and each processor has an identical */  /*-MPI-*/
  /* copy of the input file, each processor must read in (and     */  /*-MPI-*/
  /* throw away a chunk of sequences to adjust the file pointer   */  /*-MPI-*/
  /* to the correct place.                                        */  /*-MPI-*/
  /****************************************************************/  /*-MPI-*/
  for (ith = 1; ith < MPINodeStartPosn(msets); ith++) {            /*-MPI-*/
    doinput();                                                       /*-MPI-*/
    if (ith == 1) firstset = false;                                   /*-MPI-*/
  }                                                                   /*-MPI-*/
  MPI_Barrier(MPI_COMM_WORLD);                                        /*-MPI-*/
  for (ith = MPINodeStartPosn(msets);                              /*-MPI-*/
       ith < MPINodeStartPosn(msets)+MPINodeWorkAmt(msets);     /*-MPI-*/
        ith++) {                                                      /*-MPI-*/
  /* for (ith = 1; ith <= msets; ith++) { */                         /*-Orig-*/
    doinput();
    if (ith == 1)
      firstset = false;
    if (msets > 1 && !justwts) {
  ...
  /***********************************************************************/
  MPI_Barrier(MPI_COMM_WORLD);                                    /*-MPI-*/
  MPICollectOutputFiles(OUTFILE,"output file", argv[0]);          /*-MPI-*/
  MPI_Barrier(MPI_COMM_WORLD);                                    /*-MPI-*/
  MPICleanupFiles(OUTFILE);                                       /*-MPI-*/
  MPICleanupFiles(INFILE);                                        /*-MPI-*/
  if (weights || justwts) MPICleanupFiles(WEIGHTFILE);            /*-MPI-*/
  if (trout) {                                                    /*-MPI-*/
     MPI_Barrier(MPI_COMM_WORLD);                                 /*-MPI-*/
     MPICollectOutputFiles(OUTTREE,"output tree file", argv[0]);  /*-MPI-*/
     MPI_Barrier(MPI_COMM_WORLD);                                 /*-MPI-*/
     MPICleanupFiles(OUTTREE);                                    /*-MPI-*/
     }                                                            /*-MPI-*/
  FClose(stdin);                                                  /*-MPI-*/
  MPICleanupFiles("stdin");                                       /*-MPI-*/
  MPI_Barrier(MPI_COMM_WORLD);                                    /*-MPI-*/
  MPI_Finalize();                                                 /*-MPI-*/
  return 0;
}  /* Protein parsimony by uphill search */
