...
{  /* DNA Maximum Likelihood */
  ...
  MPICopyWholeFile("stdin", "stdin file", "MPIdnaml");              /*-MPI-*/
  MPI_Barrier(MPI_COMM_WORLD);                                      /*-MPI-*/
 
  MPIOpenStdinFile(stdin,"stdin","stdin file","r", "MPIdnaml",      /*-MPI-*/
              stdinfilename);                                       /*-MPI-*/
  MPI_Barrier(MPI_COMM_WORLD);                                      /*-MPI-*/
  /***************************************************************** *-MPI-*/
  init(argc,argv);
  ...
  ttratio0 = ttratio;
  if (ctgry) {
    MPICopyWholeFile(CATFILE, "categories file", "MPIdnaml");     /*-MPI-*/
    MPI_Barrier(MPI_COMM_WORLD);                                     /*-MPI-*/
    MPIOpenFile(&catfile,CATFILE,"categories file", "r", argv[0],    /*-MPI-*/
              catfilename);                                          /*-MPI-*/
    MPI_Barrier(MPI_COMM_WORLD);                                     /*-MPI-*/
    /*openfile(&catfile,CATFILE,"categories file","r",argv[0],catfilename);*//*orig*/
    }
  if (weights || justwts){
    MPICopyWholeFile(WEIGHTFILE, "weights file", "MPIdnaml");     /*-MPI-*/
    MPI_Barrier(MPI_COMM_WORLD);                                     /*-MPI-*/
    MPIOpenFile(&weightfile,WEIGHTFILE,"weights file", "r", argv[0], /*-MPI-*/
              weightfilename);                                       /*-MPI-*/
    MPI_Barrier(MPI_COMM_WORLD);                                     /*-MPI-*/
    /*openfile(&weightfile,WEIGHTFILE,"weights file","r",argv[0],weightfilename);*/ /*orig*/
    }
  if (trout){
    MPI_Barrier(MPI_COMM_WORLD);                                     /*-MPI-*/
    MPIOpenFile(&outtree,OUTTREE,"output tree file", "w", argv[0],   /*-MPI-*/
              outtreename);                                          /*-MPI-*/
    MPI_Barrier(MPI_COMM_WORLD);                                     /*-MPI-*/
    /*openfile(&outtree,OUTTREE,"output tree file","w",argv[0],outtreename);*/ /*orig*/
    }
  ...
  /*************************************************************** *-MPI-*/
  for (ith = 1; ith < MPINodeStartPosn(datasets); ith++) {        /*-MPI-*/
    getinput();                                                    /*-MPI-*/
    if (ith == 1) firstset = false;                               /*-MPI-*/
  }                                                               /*-MPI-*/
  for (ith = MPINodeStartPosn(datasets);                          /*-MPI-*/
       ith < MPINodeStartPosn(datasets)+MPINodeWorkAmt(datasets); /*-MPI-*/
        ith++) {                                                  /*-MPI-*/
 /* for (ith = 1; ith <= datasets; ith++) { */                /*orig*/
    if (datasets > 1) {
  ...
  if (ctgry) MPICleanupFiles(CATFILE);                            /*-MPI-*/
  MPI_Finalize();                                                 /*-MPI-*/
  return 0;
}  /* DNA Maximum Likelihood */
