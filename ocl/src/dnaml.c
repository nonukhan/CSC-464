...
const char *oclRed = 														\
"__kernel void ocl_reduce(__global double *a, __global double *b, __global double *c, __global double re, const int n) {\n" \
...
void nuview(node *p)
{
...
#ifdef TIMINGS
	// start timing after recursive calls
	get_start_time(nuview_tk);
#endif	
	...
	free (local_nvd);
#ifdef TIMINGS
	get_stop_time(nuview_tk);
#endif
}  /* nuview */


double ocl_Reduce(double *h_probcat, double *h_term) {
	double result;
	cl_mem d_a;
	cl_mem d_b;
	cl_mem d_c;
    ...
    globalSize = ceil(rcategs/(float)localSize)*localSize;

    err = clGetPlatformIDs(1, &cpPlatform, NULL);
    err = clGetDeviceIDs(cpPlatform, CL_DEVICE_TYPE_GPU, 1, &device_id, NULL);
    context = clCreateContext(0, 1, &device_id, NULL, NULL, &err);
    queue = clCreateCommandQueue(context, device_id, 0, &err);
    program = clCreateProgramWithSource(context, 1, (const char **) &oclRed, NULL, &err);
    ...
    d_a = clCreateBuffer(context, CL_MEM_READ_ONLY, rcategs*sizeof(double), NULL, NULL);
    d_b = clCreateBuffer(context, CL_MEM_READ_ONLY, rcategs*sizeof(double), NULL, NULL);
    d_c = clCreateBuffer(context, CL_MEM_WRITE_ONLY, rcategs*sizeof(double), NULL, NULL);
	d_re = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(double), NULL, NULL);
	
	// Write our data set into the input array in device memory
	err = clEnqueueWriteBuffer(queue, d_a, CL_TRUE, 0,
                                   rcategs*sizeof(double), h_probcat, 0, NULL, NULL);
    err |= clEnqueueWriteBuffer(queue, d_b, CL_TRUE, 0,
                                   rcategs*sizeof(double), h_term, 0, NULL, NULL);

	// Set the arguments to our compute kernel
    err  = clSetKernelArg(kernel, 0, sizeof(cl_mem), &d_a);
    err |= clSetKernelArg(kernel, 1, sizeof(cl_mem), &d_b);
    err |= clSetKernelArg(kernel, 2, sizeof(cl_mem), &d_c);
	err |= clSetKernelArg(kernel, 3, sizeof(cl_mem), &d_re);
    err |= clSetKernelArg(kernel, 4, sizeof(int), &rcategs);
    ...
    clFinish(queue);
	clEnqueueReadBuffer(queue, d_re, CL_TRUE, 0, sizeof(double), &result, 0, NULL, NULL);
	
	// Cleanup
	clReleaseMemObject(d_a);
    clReleaseMemObject(d_b);
    clReleaseMemObject(d_c);
	clReleaseMemObject(d_re);
...
void slopecurv(node *p,double y,double *like,double *slope,double *curve)
{
	...
	for (i = 0; i < endsite; i++) {
		...
		sumterm = ocl_Reduce(probcat, *term+(i*j*sizeof(double)));
